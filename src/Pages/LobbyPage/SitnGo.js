...
const SitnGoTable = (props) => {
  ...
  const [rake, setRake]=useState(0.95);
  ...
  useEffect(() => {
    let tmp_rake=5;
    for(let i=0;i<props.sitRakes.length-1;i++){
      console.log('buyIn='+buyIn);
      console.log('props.sitRakes[i].buyIn='+props.sitRakes[i].buyIn);
      if(buyIn>=props.sitRakes[i].buyIn && buyIn<props.sitRakes[i+1].buyIn){
        console.log('the same');
        tmp_rake=props.sitRakes[i].rake;
        break;
      }else if(buyIn>=props.sitRakes[props.sitRakes.length-1].buyIn){
        tmp_rake=props.sitRakes[props.sitRakes.length-1].rake;
      }    
    }
    tmp_rake=(100-tmp_rake)/100;
    console.log(tmp_rake);
    setRake(tmp_rake);
    setFirstPlace( Math.floor(buyIn * tableSize * tmp_rake));
    setSecondPlace(0);
    setThirdPlace(0);
  }, [tableSize, buyIn, props.sitRakes]);
        ...
        <Fade in={createModal}>
          <div className={classes.modal_paper}>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={8} className={classes.modal_field}>
              {showDot(Math.floor(buyIn * tableSize * rake))}
              </Grid>
            </Grid>
                  ...
                  <Grid item xs>
                    <Slider
                      ...
                      onChange={(e, val) => {
                        if (
                          val >
                          Math.floor(buyIn * tableSize * rake) -
                            secondPlace -
                            thirdPlace
                        )
                          setFirstPlace(
                      ...
                      min={0}
                      max={Math.floor(buyIn * tableSize * rake)}
                      color={"primary"}
