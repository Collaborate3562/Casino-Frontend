...
const CashGame = ({
  ...
  const [addChipsModal, setAddChipsModal] = useState(false);
  const [buyIn, setBuyIn] = useState(0);
  const [loading, setLoading] = useState(0);
  const [infoModal, setInfoModal] = useState(false);
  const [fairModal, setFairModal] = useState(false);
  const addChipsModalShow = () => {
    console.log("topup");
    setAddChipsModal(true);
    setStandMenu(null);
  };
  const addChips = () => {
    if (chipsAdded > 0) {
      socket.emit("cash:addChips", match.params.room, chipsAdded, (res) => {
        if (!res.status) {
          handleToast(res.message);
        } else {
          setAddChipsModal(false);
          handleToast("Top up successfully!", success);
          setTable((table) => {
            const players = table.players.map((ele) => ele);
            players[mySeat].balance = res.balance;
            setTable({
              ...table,
              players,
            });
  ...
  const raiseInc = () => {
    setRaise((prev) => {
      return prev + (table.blinds < 100 ? 1 : table.blinds / 10) < maxRaise
        ? prev + (table.blinds < 100 ? 1 : table.blinds / 10)
        : prev;
    });
  };
  const raiseDec = () => {
    setRaise((prev) => {
      return prev - (table.blinds < 100 ? 1 : table.blinds / 10) > minRaise
        ? prev - (table.blinds < 100 ? 1 : table.blinds / 10)
        : prev;
    });
  };
      ...
      socket.on(
        ...
        ({
          ...
          setTable((table) => {
            ...
            newPlayers[position].betStatus = betStatus;
      ...
      <div className={classes.root}>
        ...
        >
          <div className={classes.menu_bar}>
            ...
            >
              <MenuItem onClick={() => setFairModal(true)}>Fair</MenuItem>
              <MenuItem onClick={gotoLobby}>Lobby</MenuItem>
              <MenuItem onClick={logout}>Logout</MenuItem>
            </Menu>
            <div className={classes.balance_pad}>
              <span>
                Balance : {showDot(credential.loginUserDoge)}{" "}
                <small>credits</small>
              </span>
              {table &&
          ...
          <div className={classes.table_pad}>
            ...
            >
              {table &&
                ...
                table.players[mySeat].balance < table.buyIn[1] ? (
                <MenuItem onClick={addChipsModalShow}>Top up</MenuItem>
              ) : (
                ""
              )}
            ...
            <div className={classes.table_cards + " table-cards"}>
                ...
                {table.tableCards
                  ? table.tableCards.map((ele, key) => (
                    <React.Fragment key={key}>
                      {getCards(
                        getCardSrc(ele + ""),
                        "card-size animate__animated animate__flipInY animate__slower"
                      )}
                    </React.Fragment>
                  ))
                  : ""}
              ...
              <div className="chips-pad">
                ...
                <div className="chips-amount total">
                  {table.pot > 0 ? showKilo(table.pot) : ""}
                </div>
              </div>
            </div>
            <Hidden xsDown>
                ...
                {callStatus == "allIn" ? (
                  ...
                  <React.Fragment>
                    <Button color="info" onClick={() => bet("call", call)}>
                      {call - table.players[mySeat].bet > 0
                        ? "Call " + showKilo(call - table.players[mySeat].bet)
                        : "Check"}
                    </Button>
                    <Button color="danger" onClick={() => bet("allIn")}>
                      All In
                  ...
                  <React.Fragment>
                    ...
                    <PrettoSlider
                      ...
                      min={minRaise}
                      max={maxRaise}
                      step={table.blinds < 100 ? 1 : table.blinds / 10}
                      color={"primary"}
            ...
            <Fade in={passwordModal}>
              <div className={classes.modal_paper}>
                ...
                <div className={classes.modal_body}>
                  ...
                  <Grid container spacing={3}>
                    <Grid item className={classes.modal_center}>
                      Your Balance : {credential.loginUserDoge}
                    </Grid>
                  </Grid>
                  <Grid container spacing={3}>
                    <Grid item xs={12} className={classes.modal_center}>
                      <Button
                        ...
                        onClick={() =>
                          setBuyIn((prev) => {
                            return buyIn -
                              (table.blinds < 100 ? 1 : table.blinds / 10) >
                              table.buyIn[0]
                              ? buyIn -
                              (table.blinds < 100 ? 1 : table.blinds / 10)
                              : buyIn;
                      ...
                      <PrettoSlider
                        ...
                        min={table.buyIn[0]}
                        max={
                          table.buyIn[1] > credential.loginUserDoge
                            ? credential.loginUserDoge
                            : table.buyIn[1]
                        }
                        step={table.blinds < 100 ? 1 : table.blinds / 10}
                        color={"primary"}
            ...
            <Fade in={infoModal}>
              <div className={classes.small_modal_paper}>
                ...
                <div className={classes.modal_body}>
                  ...
                  <Grid container spacing={3}>
                    <Grid item className={classes.modal_center}>
                      <Button color="doge" onClick={copyLink}>
                        Invite Link
                      </Button>
                    </Grid>
                  </Grid>
        ...
        {table &&
          ...
          <Modal
            ...
            aria-describedby="transition-modal-description"
            open={addChipsModal}
            onClose={() => setAddChipsModal(!addChipsModal)}
            closeAfterTransition
            BackdropComponent={Backdrop}
            BackdropProps={{
              timeout: 500,
              ...
              <div className={classes.modal_paper}>
                <h4 className={classes.modal_title}>
                  ...
                  <Button
                    ...
                    justIcon
                    className={classes.modal_close}
                    onClick={() => setAddChipsModal(false)}
                  >
                    <AiOutlineClose />
                  </Button>
                </h4>
                <div className={classes.modal_body}>
                  <Grid container spacing={3}>
                    <Grid item className={classes.modal_center}>
                      <h5>
                        How many chips would you like to bring more to the
                        table?
                      </h5>
                    </Grid>
                  </Grid>
                  ...
                  <Grid container spacing={3}>
                    <Grid item xs={12} className={classes.modal_center}>
                      ...
                      <PrettoSlider
                        value={chipsAdded}
                        onChange={(e, val) => setChipsAdded(val)}
                        valueLabelDisplay="on"
                        aria-labelledby="range-slider"
                        ValueLabelComponent={ValueLabelComponent}
                        getAriaValueText={(val) => showKilo(val)}
                        ...
                        min={0}
                        max={
                          table.buyIn[1] - table.players[mySeat].balance >
                            credential.loginUserDoge
                            ? credential.loginUserDoge
                            : table.buyIn[1] - table.players[mySeat].balance
                        }
        ...
        {table ? (
          <Modal
            ...
            aria-describedby="transition-modal-description"
            open={fairModal}
            onClose={() => setFairModal(!fairModal)}
            closeAfterTransition
            BackdropComponent={Backdrop}
            BackdropProps={{
              timeout: 500,
                ...
                <div className={classes.modal_body}>
                  ...
                  <Grid container spacing={3}>
                    ...
                    <Grid
                      item
                      xs={8}
                      className={classes.modal_field}
                      style={{ overflow: "auto" }}
                    >
                      <span style={{ width: "100%" }}>{table.serverHash}</span>
                    </Grid>
                  </Grid>
                  <Grid container spacing={3}>
                    <Grid item xs={4} className={classes.modal_label}>
